  ...
  <card :content-style="{ height: '330px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }">
    <template #title>
      <el-radio-group v-model="params.type" @change="getData">
        <el-radio-button :value="1">充值统计</el-radio-button>
        <el-radio-button :value="2">提现统计</el-radio-button>
      </el-radio-group>
    </template>
    <template #extra>
      <div>
...
import { getRechargeWithdrawalStatistics } from "@/api/modules/home";
...
const params = reactive<{ type: 1 | 2; date: string[] }>({
  type: 1,
...
const getData = () => {
  ...
  getRechargeWithdrawalStatistics({ type: params.type, start: params.date[0], end: params.date[1] })
    .then((res: any) => {
      ...
      console.log(res, "getRechargeWithdrawalStatistics");
    })
    .finally(() => {
